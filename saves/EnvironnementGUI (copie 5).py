...
EVAP = 20  # Vitesse d'évaporation des traces de phéromone (itérations)

##### NE PAS MODIFIER :
STATE = ["FORAGING", "GOBACK", "WORKING"]  # États possibles pour chaque fourmi
...
class Ants(Mobile):
        ...
        for t in fen.envi.trails:
            if round(self.posX, 6) == round(t[0], 6) \
               and round(self.posY, 6) == round(t[1], 6):
                if random() < 0.75:
                    try:
                        next_trail = fen.envi.trails[fen.envi.trails.index(t)+len(fen.envi.ants)]
    ...
    def check_pos(self):
        """
            ...
            Redéfinie la trajectoire vers le dernier point de nourriture trouvé.
            Ne renvoie rien.
        """
