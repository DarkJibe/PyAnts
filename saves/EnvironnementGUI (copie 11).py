...
class Ants():
    ...
    def go_to(self, pos):
        ...
        try:
            ...
            self.vitesseX = (pos[0] - self.posX) / 0.001
            self.vitesseY = (pos[1] - self.posY) / 0.001
        self.normalise()  # On normalise les trajectoire
    ...
    def check_others(self):
        ...
        for ant in other_ants:
            if self.check_pos([ant.posX, ant.posY]):
                self.change_dir2()
    ...
    def check_pos(self, pos):
        """
            ...
            Renvoie un booléen.
        """
        if self.posX >= pos[0]-ACUITY and self.posX <= pos[0]+ACUITY \
           and self.posY >= pos[1]-ACUITY and self.posY <= pos[1]+ACUITY:
            return True
        return False
        ...
        if self.avoid_obstacles(obs):   # S'il y a un obstacle,
            ...
            if self.state == STATE[0]:  # SI FORAGING :
                if self.check_trail():  # Elle verifie les traces de phéromone
                    self.check_others()  # puis la colision avec les autres
