...
STATE = ["FORAGING", "GOBACK", "FOLLOW"]  # États possibles pour chaque fourmi
EVAP = 5

class Mobile(object):
    def __init__(self, x, y):
        self.posX = float(x)  # Positions X et Y du Mobile dans l'environnement
        self.posY = float(y)
...
class Ants(Mobile):
    ...
    def go_back(self):
        """
            ...
            Ne renvoie rien.
        """
        anth_pos = fen.envi.anthill  # Pos de la fourmilière
        distance = sqrt((anth_pos[0]-self.posX)**2
                       +(anth_pos[1]-self.posY)**2)  # Distance fourmilière-fourmi
        f = distance / PAS  # Nb d'itérations pour atteindre le pt d'arrivée
        self.vitesseX = (anth_pos[0] - self.posX) / f
        self.vitesseY = (anth_pos[1] - self.posY) / f
        self.normalise()  # On normalise les trajectoire
...
class Environment():
    ...
    def __init__(self, nb_ants, qt_food, obstacles, largeur, hauteur):
        ...
        self.anthill = [random() * float(largeur),  # posX et
                       random() * float(hauteur)]   # posY de la fourmilière
        ...
        for i in range(qt_food):
            self.food.append([randint(0, largeur), randint(0, hauteur),  # Position des points de nourriture
                              randint(20, 50), randint(20, 50)])  # Diamètre des points de nourriture
        for i in range(obstacles):
            self.obstacles.append([randint(0, largeur), randint(0, hauteur),  # Position des obstacles
                                   randint(20, 50), randint(20, 50)])  # Diamètre des obstacles
    ...
    def maj_trails(self):
        """
            Met à jour la qualité des traces de phéromone laissées par les fourmis.
        """
        for ant in self.ants:
            self.trails.append([ant.posX, ant.posY])  # On ajoute les positions
        if len(self.trails) > EVAP * self.nb_ants:
            for i in range(nb_ants):
                del self.trails[i]  # Supprime les traces les plus anciennes
...
# À ÉCRIRE self.maj_trail()  # Met à jour les traces de phéromone
...
class Window(QWidget):
    ...
    def initUI(self):  # Crée la GUI
        self.envi = Environment(5, 5, 10, 800, 600)
